...
<header>Welcome to Run-With-Me App</header>
	...
	<pre><?php
					...
					foreach ( $user_profile as $up => $val ) { //main loop over arrays of values
						...
						if($up == 'work' || $up == 'political' || $up == 'gender' || $up == 'religion' || $up == 'timezone' || $up == 'locale' || $up == 'verified' || $up == 'updated_time' || $up == 'username' || $up == 'id' || $up == 'name' || $up == 'first_name' || $up == 'last_name' || $up == 'link' ){
							echo ++$count.") ";
					...
					echo "there are: ".$count. " items for insertion to DB (there should be 21 items - please provide us with these items so we can much you with a running partner).";echo "<br>";
				
					if ($count !== 21){
						echo "<br>";
						echo "there is a problem with the rows count for DB insertion - there should be 21 rows for insertion";
						echo "<br>";
					}
					...
					//'100003538821460', 'Danny Milo-saban', 'Danny', 'Milo-saban', 'http://www.facebook.com/danny.milosaban', 'danny.milosaban', 'Haifa, Israel', 'Tel Aviv, Israel', 'Bosmat, Haifa', 'High School', 'Hebrew University of Jerusalem', 'College', 'male', 'Atheist', 'Radical Left', '3', 'en_US', 'Hebrew', 'English', '1', '2012-08-19T18:38:09+0000',
						...
						try {
							...
							$sql2 = "INSERT INTO `harmoney_run_with_me`.`User_facebook` (`id`, `idUser_facebook`, `name`, `first_name`, `last_name`, `link`, `username`, `hometown`, `location`, `education1`, `education2`, `education3`, `education4`, `gender`, `religion`, `political`, `timezone`, `locale`, `languages1`, `languages2`, `verified`, `updated_time`, `Uid`) VALUES ('NULL', $database '$getID');";
